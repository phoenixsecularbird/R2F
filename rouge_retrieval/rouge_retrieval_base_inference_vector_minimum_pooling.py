...
class NET(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, tag):
        ...
        logits = logits.reshape(batch_size, num_h_sents, -1)

        temp = torch.ones_like(logits).cuda() * 9e15

        tag = tag.reshape(batch_size, num_h_sents, 1)

        logits = tag * logits + (1 - tag) * temp

        min_logits = torch.min(logits, dim=1).values

        min_logits = self.linear(min_logits).view(-1)

        logits = self.linear(logits).reshape(batch_size, num_h_sents)
