...
def convert_to_features(tokenizer, in_file, max_seq_length, metrics, max_h_sens, max_evi_sens):
    ...
    with open(in_file) as f:
        samples = json.load(f)
    rouge_scorer = Rouge([metrics])

    def get_rouge_score(sen0, sen1):
        return rouge_scorer.get_scores(sen0, sen1)[0][metrics]["f"]
    ...
    def get_evidence_sentences(sen0, sens):
        ...
        for sen in sens:
            try:
                score = get_rouge_score(sen0, sen)
            except:
                score = 0.0
            scores.append(score)

        _max_evi_sens = min(max_evi_sens, len(sens))
        index = torch.topk(torch.tensor(scores), k=_max_evi_sens).indices.tolist()
        evis = [sens[_] for _ in sorted(index)]
        return evis
...
class KERNEL(nn.Module):
    ...
    def __init__(self, kernel_size=11, sigma_square=0.01):
        super(KERNEL, self).__init__()

        self.mu = torch.tensor([0.1 * i for i in range(kernel_size)]).view(1, -1).cuda()

        self.sigma_square = sigma_square

        self.aggregate = nn.Linear(in_features=11, out_features=1)

    def forward(self, logits, tag):

        batch_size, num_sents = logits.size()

        logits = torch.sigmoid(logits).reshape(-1, 1)

        logits = torch.exp(- (logits - self.mu) ** 2 / self.sigma_square)

        logits = logits.reshape(batch_size, num_sents, -1)

        logits = torch.sum(logits * tag.unsqueeze(dim=2), dim=1)

        logits /= torch.sum(tag, dim=1, keepdim=True)

        logits = self.aggregate(logits).view(-1)
...
class NET(nn.Module):
    ...
    def __init__(self, model_name_or_path, kernel_size=11, sigma_square=0.01):
        ...
        self.kernel = KERNEL(kernel_size=kernel_size, sigma_square=sigma_square)

    def forward(self, input_ids, attention_mask, tag):

        batch_size, num_sents, seq_length = input_ids.size()

        input_ids = input_ids.view(batch_size * num_sents, seq_length)

        attention_mask = attention_mask.view(batch_size * num_sents, seq_length)

        logits = self.roberta(input_ids=input_ids, attention_mask=attention_mask).last_hidden_state[:, 0]

        logits = self.linear(logits)

        logits = logits.reshape(batch_size, num_sents)

        min_logits = self.kernel(logits, tag)

        return min_logits, None
