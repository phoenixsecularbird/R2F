...
def convert_to_features(tokenizer, in_file, max_seq_length, metrics, max_h_sens, max_evi_sens):
    ...
    retrieval_model = RobertaModel.from_pretrained(r"unsup-simcse-roberta-base").cuda()
    retrieval_tokenzier = RobertaTokenizer.from_pretrained(r"unsup-simcse-roberta-base")
    ...
    for item in samples:
        ...
        if len(premise) > max_evi_sens:
            embeded_h = [get_embeddings(tokenizer=retrieval_tokenzier, model=retrieval_model,
                                        texts=hypothesis[i * 64: (i + 1) * 64])
                         for i in range(len(hypothesis) // 64 + 1) if hypothesis[i * 64: (i + 1) * 64]]

            embeded_h = torch.cat(embeded_h, dim=0)

            embeded_p = [get_embeddings(tokenizer=retrieval_tokenzier, model=retrieval_model,
                                        texts=premise[i * 64: (i + 1) * 64])
                         for i in range(len(premise) // 64 + 1) if premise[i * 64: (i + 1) * 64]]

            embeded_p = torch.cat(embeded_p, dim=0)
            scores = torch.cosine_similarity(embeded_h.unsqueeze(dim=1), embeded_p.unsqueeze(dim=0), dim=-1)
        ...
        for i, sen in enumerate(hypothesis):
            if len(premise) > max_evi_sens:
                idx = torch.topk(scores[i], k=5).indices.tolist()
                related_sen = [premise[_] for _ in sorted(idx)]
            else:
                related_sen = premise
    ...
    del retrieval_model, retrieval_tokenzier
...
def main():
    in_file = r""  # sentence-level annotation file
