...
def convert_to_features(tokenizer, in_file, max_seq_length, metrics, max_h_sens, max_evi_sens):
    ...
    def get_evidence_sentences(bm25_model, sen0, sens):

        sen0 = sen0.split()

        scores = bm25_model.get_scores(sen0)
    ...
    for item in samples:
        ...
        corpus = [_.split() for _ in premise]
        ...
        for i, sen in enumerate(hypothesis):
            related_sen = get_evidence_sentences(bm25_model, sen, premise) if len(premise) > max_evi_sens else premise
